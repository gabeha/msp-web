...
<div>
    ...
    <ul :id="'sidebar-menu-'+title" class="hidden py-2 space-y-2 rounded-lg">
            ...
            <li v-for="neu in neuroscience" :key="neu.id">
                <button @click="this.$emit('course-clicked', neu.id)" class="flex items-center p-2 pl-11 w-full text-base font-normal text-gray-900 transition duration-75 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 border-b border-gray-200">{{neu.subject}}{{neu.code}}</button>
            </li>
            <li v-for="che in chemistry" :key="che.id">
                <button @click="this.$emit('course-clicked', che.id)" class="flex items-center p-2 pl-11 w-full text-base font-normal text-gray-900 transition duration-75 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 border-b border-gray-200">{{che.subject}}{{che.code}}</button>
            </li>
            <li v-for="int in interdisciplinary" :key="int.id">
                <button @click="this.$emit('course-clicked', int.id)" class="flex items-center p-2 pl-11 w-full text-base font-normal text-gray-900 transition duration-75 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 border-b border-gray-200">{{int.subject}}{{int.code}}</button>
            </li>
            <li v-for="pra in practicals" :key="pra.id">
                <button @click="this.$emit('course-clicked', pra.id)" class="flex items-center p-2 pl-11 w-full text-base font-normal text-gray-900 transition duration-75 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 border-b border-gray-200">{{pra.subject}}{{pra.code}}</button>
            </li>
    </ul>
