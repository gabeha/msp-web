<template>
    <div @click="emitPeriod" class="cursor-pointer flex flex-col text-center items-center min-h-period rounded-lg border shadow-md py-3 px-3 bg-white bg-opacity-80" :class="this.current[0] == this.semester && this.current[1] == this.periodNumber ? 'border-amber-500 border-2 shadow-inner' : ''">
        <h5  class="select-none font-medium leading-tight text-base mt-0 mb-2 text-gray-600 rounded-xl border border-gray-300 px-2 pb-1.5 pt-1 ">Period {{periodNumber}}</h5>
        <div v-if="choices !== undefined">
            <div v-for="c in choices.filter(choice => (choice.semester == this.semester && choice.period == this.periodNumber))" :key="c.selectedModule.id" class="flex flex-col justify-between">
                    ...
                    :class="c.selectedModule.hasprerec ? '' : ''">
                            ...
                            <button class="text-left font-medium text-xs"
